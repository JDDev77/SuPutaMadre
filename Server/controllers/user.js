...
const updateOp = async (req, res) => {
    const userId = req.params.id;  // Captura el ID desde el parámetro de la ruta
    let updates = req.body;

    // Opcional: Validar los datos entrantes antes de proceder con la actualización
    try {
        validate(updates);  // Asegúrate de que esta función maneja la validación adecuadamente
    } catch (error) {
        return res.status(400).send({
            status: "error",
            message: "Datos de actualización no válidos: " + error.message
        });
    }

    // Prevenir actualización de campos no permitidos
    delete updates.password;  // Si quisieras permitir actualización de contraseña, deberías manejarlo aparte
    delete updates.email;  // Asimismo, se podría considerar manejar el email de manera especial si es necesario
